...
if params["ERROR_COMPUTATION"] == "single-ride"
	@time status, new_times, directory = fast_LP(manhattan, travel_times, num_rides, trainTestDf, startTimes, errorComputation=params["ERROR_COMPUTATION"], model_type=params["MODEL"], last_smooth=params["LAST_SMOOTH"], max_rounds=params["MAX_ROUNDS"], min_rides=params["MIN_RIDES"], radius=params["RADIUS"], year=params["YEAR"], startTime=params["START_TIME"], endTime=params["END_TIME"], startMonth=params["START_MONTH"], endMonth=params["END_MONTH"], method=params["METHOD"], sample_size=params["SAMPLE_SIZE"], turnCost=params["TURN_COST"], turnCostAsVariable=params["TURN_COST_AS_VARIABLE"], delta_bound=params["DELTA_BOUND"], maxNumPathsPerOD=params["MAX_NUM_PATHS_PER_OD"], computeFinalSP = params["COMPUTE_FINAL_SHORTEST_PATHS"], startWithSimpleLP=params["START_SIMPLE"], randomConstraints=params["RANDOM_CONSTRAINTS"], dynamicConstraints=params["DYNAMIC_CONSTRAINTS"], numPairsToAdd = params["NUM_OD_ADDED"], iterationMultiple = params["UPDATE_EVERY_N_ITERATIONS"], metropolis=params["METROPOLIS"])
